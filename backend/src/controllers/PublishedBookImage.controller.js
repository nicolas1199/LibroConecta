...
export async function addImageToPublishedBook(req, res) {
  try {
    ...
    if (is_primary) {
      await PublishedBookImage.update({ is_primary: false }, { where: { published_book_id: publishedBookId } })
    }
...
export async function updatePublishedBookImage(req, res) {
  try {
    ...
    // Si se marca como primaria, desmarcar otras imágenes primarias
    if (is_primary) {
      await PublishedBookImage.update({ is_primary: false }, { where: { published_book_id: image.published_book_id } })
    }
