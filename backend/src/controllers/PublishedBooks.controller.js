...
export async function createPublishedBook(req, res) {
  try {
    ...
    res.status(500).json({ error: "Error al crear libro publicado", details: error.message })
  }
}
...
export async function updatePublishedBook(req, res) {
  try {
    const { id } = req.params
    const { transaction_type_id, price, look_for, condition_id, location_id, description } = req.body
