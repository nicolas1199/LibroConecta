...
export const updatePublishedBook = async (publishedBookId, updateData) => {
  try {
    ...
    console.error("Error en updatePublishedBook:", error.response?.data || error.message)
    throw error
  }
}
...
export const searchPublishedBooks = async (searchTerm, page = 1, limit = 20) => {
  const res = await api.get("/published-books/search", {
    params: {
      q: searchTerm,
      page,
      limit
